    ...
    stage('default-0') {
      parallel {
        stage('拉取代码') {
          agent none
          steps {
            git(credentialsId: 'gitlab-account', url: 'http://gitlab.xxx.com/idp-group/system.git', branch: '$BRANCH_NAME', changelog: true, poll: false)
      ...
      steps {
        container('maven') {
          withCredentials([usernamePassword(credentialsId : 'harbor-account' ,usernameVariable : 'DOCKER_USER_VAR' ,passwordVariable : 'DOCKER_PWD_VAR' ,)]) {
            ...
            sh '''docker tag  idp-system:$BUILD_TAG $REGISTRY/$DOCKERHUB_NAMESPACE/idp-system:$BUILD_TAG
docker push  $REGISTRY/$DOCKERHUB_NAMESPACE/idp-system:$BUILD_TAG'''
          }

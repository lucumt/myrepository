    ...
    stage('初始阶段') {
      parallel {
        stage('拉取代码') {
          agent none
          steps {
            git(credentialsId: 'gitlab-account', url: 'http://gitlab.xxx.com/idp-group/idp-common-web.git', branch: '$BRANCH_NAME', changelog: true, poll: false)
            sh '''ls -al
echo “拉取代码成功“'''
          }
        }
...
npm run build:${PRODUCT_PHASE}'''
    ...
    stage('镜像构建') {
      ...
      steps {
        container('nodejs') {
          sh 'docker build -f kubesphere/Dockerfile -t idp-common-web:$BUILD_TAG .'
        }
      ...
      steps {
        container('nodejs') {
          withCredentials([usernamePassword(credentialsId : 'harbor-account' ,usernameVariable : 'DOCKER_USER_VAR' ,passwordVariable : 'DOCKER_PWD_VAR' ,)]) {
            ...
            sh '''docker tag  idp-common-web:$BUILD_TAG $REGISTRY/$DOCKERHUB_NAMESPACE/idp-common-web:$BUILD_TAG
docker push  $REGISTRY/$DOCKERHUB_NAMESPACE/idp-common-web:$BUILD_TAG'''
          }
    ...
    stage('部署到dev') {
      agent none
      steps {
        kubernetesDeploy(enableConfigSubstitution: true, deleteResource: false, kubeconfigId: 'idp-kubeconfig', configs: 'kubesphere/deploy.yml')
      }
    }
